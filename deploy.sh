...
case "$1" in
    ...
    fix-dns)
        ...
        ;;
    replace-self)
        print_warning "This command helps you overwrite the current script content."
        echo "Please run: sudo tee deploy.sh << 'EOF'"
        echo "(Paste new content here)"
        echo "EOF"
        echo ""
        ;;
    *)
        echo "Usage: $0 {setup|up|down|logs|update|seed|fix-dns|replace-self}"
        echo ""
