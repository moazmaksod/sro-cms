        ...
        <div class="card-body">
            <nav class="mb-4">
                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                    @foreach(['all' => 'All News', 'news' => 'News', 'update' => 'Updates', 'event' => 'Events'] as $key => $label)
                                        ...
                                        <div class="card h-100">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <a href="{{ route('news.show', ['slug' => $row->slug]) }}" class="text-decoration-none">
                                                    <h3 class="card-title fw-bold h5">{{ \Illuminate\Support\Str::words(strip_tags($row->title), 3, '...') }}</h3>
                                                </a>
                                                <div class="card-text">{{ \Illuminate\Support\Str::words(strip_tags($row->content), 20, '...') }}</div>
                                                <a href="{{ route('news.show', ['slug' => $row->slug]) }}" class="text-decoration-none mt-4">Read More →</a>
                                            </div>
                                        </div>
                                    </div>
                                @endif
